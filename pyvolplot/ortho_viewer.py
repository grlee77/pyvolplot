""" minor modification of OrthoSlicer3D from the following nibabel PR:
https://github.com/nipy/nibabel/pull/251

The OrthoSlicer3D code was originally written by Paul Ivanov.

This code is covered by the MIT license as detailed here:
...
class OrthoSlicer3Dv2(OrthoSlicer3D):
    ...
    def _on_keypress(self, event):
        ...
        super(OrthoSlicer3Dv2, self)._on_keypress(event)
        """new keypress events"""
        if event.key in ["=", '+']:
            # increment volume index
            new_idx = min(self._data_idx[3] + 1, self.n_volumes)
            ...
            if self._data is self._data1:
                self._data = self._data2
                vmin, vmax = self._data2_range
            elif self._data is self._data2:
                self._data = self._data1
                vmin, vmax = self._data1_range
            else:
                raise ValueError("unexpected data pointer")
            # use _set_volume_index to update _current_vol_data
            self._set_volume_index(self._data_idx[3], update_slices=True,
                                   irange=[vmin, vmax])
            self._draw()

    def _set_volume_index(self, v, update_slices=True, irange=None):
        """Set the plot data using a volume index
        if update_slices and irange is not None, update the displayed intensity
        range """
        v = self._data_idx[3] if v is None else int(round(v))
        if v == self._data_idx[3] and irange is None:
            return
        max_ = np.prod(self._volume_dims)
        self._data_idx[3] = max(min(int(round(v)), max_ - 1), 0)
        idx = (slice(None), slice(None), slice(None))
        ...
        # update all of our slice plots
        if update_slices:
            self._set_position(None, None, None, notify=False, irange=irange)

    def _set_position(self, x, y, z, notify=True, irange=None):
        """Set the plot data using a physical position
        if irange is not None, update the displayed intensity range """
        # deal with volume first
        if self._changing:
            return
        self._changing = True
        ...
        for ii in range(3):
            ...
            if irange is not None:
                # self._ims[ii].changed()  #necessary?
                self._ims[ii].set_clim(irange[0], irange[1])
            # deal with crosshairs
            loc = self._data_idx[ii]
            if self._flips[ii]:
                loc = self._sizes[ii] - loc
