...
#unlike the dipy version, this one works with complex data and does not require an affine

def resample_image(data,zooms,new_zooms,affine=None,order=1, mode='constant', cval=0):
    ...
    if data.ndim==3:
        if not np.iscomplexobj(data):
            data2=affine_transform(input=data,matrix=R,offset=np.zeros(3,),output_shape=tuple(new_shape),order=order, mode=mode, cval=cval)
        else:
            data2=affine_transform(input=data.real,matrix=R,offset=np.zeros(3,),output_shape=tuple(new_shape),order=order, mode=mode, cval=cval) + \
                  1j*affine_transform(input=data.imag,matrix=R,offset=np.zeros(3,),output_shape=tuple(new_shape),order=order, mode=mode, cval=cval)
    if data.ndim==4:
        data2l=[] 
        for i in range(data.shape[-1]):
            if not np.iscomplexobj(data):
                tmp=affine_transform(input=data[...,i],matrix=R,offset=np.zeros(3,),output_shape=tuple(new_shape),order=order, mode=mode, cval=cval)
            else:
                tmp=affine_transform(input=data[...,i].real,matrix=R,offset=np.zeros(3,),output_shape=tuple(new_shape),order=order, mode=mode, cval=cval) + \
                    +1j*affine_transform(input=data[...,i].imag,matrix=R,offset=np.zeros(3,),output_shape=tuple(new_shape),order=order, mode=mode, cval=cval)
            data2l.append(tmp)        
...
#    if affine is None:
#        affine=np.asarray(np.diag(list(zooms)+[1,]),dtype=np.float32)
#        print(affine)
